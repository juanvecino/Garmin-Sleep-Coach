...
# Get the parent directory (which should contain 'garmindb')
...
from garmindb import GarminUserSettings, GarminSocialProfile, GarminPersonalInformation, GarminWeightData, \
    GarminSummaryData, GarminMonitoringFitData, GarminSleepFitData, \
...
from garmindb import PluginManager, GarminConnectConfigManager
...
def download_data(overwite, latest, stats):
    ...
    if Statistics.monitoring in stats:
        date, days = __get_date_and_days(MonitoringDb(db_params_dict), latest, MonitoringHeartRate,
    ...
    if Statistics.rhr in stats:
        date, days = __get_date_and_days(GarminDb(db_params_dict), latest, RestingHeartRate,
...
def main(argv):
    ...
    parser.add_argument("--config", help="Specify the path to the configuration file", dest="config_file", default=None)
    ...
    stats_group.add_argument("-A", "--all", help="Download and/or import data for all enabled stats.",
                             dest='all_stats', action='store_true', default=False)
    ...
    gc_config = GarminConnectConfigManager(config_file=args.config_file)
    db_params_dict = gc_config.get_db_params()
    plugin_manager = PluginManager(gc_config.get_plugins_dir(), db_params_dict)

    if args.all_stats:
        args.stats = gc_config.enabled_stats()
    elif not args.stats:
        args.stats = []
